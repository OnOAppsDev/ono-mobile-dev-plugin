...
`skills/dev-design-start/SKILL.md:71` obliges each platform lane to name **its own** verify-later mechanism; `standards/android/` has no applicability section to point at, so it is defined here. The three shared risk classes and the verify-later principle stay as written in § *Shared planning rules*.
...
- **No `AND-*` equivalent exists for accessibility, i18n/RTL or security** — cite those from the shared roots above. `AND-UI-RES-*` covers resources, localisation and RTL *inside the Android UI layer*, a different thing from `I18N-*`; cite both where both apply.
