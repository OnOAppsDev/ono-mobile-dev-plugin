...
3. Once the platform is confirmed by the user (step 2), `repo-analyst` fills in the stack picture for the confirmed platform. **Every authored lane reuses canonical repository knowledge — the lanes differ only in which layer does the reusing.** The resolution procedure, the reusable-versus-derive-live decision, and the citation shape all belong to the `repo-knowledge-consumer` skill; none of it is restated here.
   ...
   - **iOS and Android — reused at the architect.** `repo-analyst` performs lightweight existence checks only (iOS: SPM vs. CocoaPods, whether an MVVM/Coordinator-style layout is present; Android: Gradle Kotlin DSL vs. Groovy, Compose vs. XML views). That is deliberate and **is not a gap in canonical-knowledge reuse**: `ios-architect` and `android-architect` each resolve canonical repository knowledge themselves through `repo-knowledge-consumer` (their Process step 2), then run their own deeper platform inspection per `skills/ios-dev-planning/SKILL.md` §3 and `skills/android-dev-planning/SKILL.md` §3. For these two lanes, treat `repo-analyst`'s Stack Detection as a starting signal, never as the authoritative picture.
   - **React (web) — deferred, because the lane is not authored.** `repo-analyst` performs lightweight existence checks only (which bundler/framework, which routing library). Unlike iOS and Android there is **no deeper inspection to defer to**: `react-architect` and `react-dev-planning` are structure-only placeholders, so deep convention detection waits until that lane and `standards/react/*` are authored (`REACT-002`). Step 4's readiness gate stops the run before an architect is invoked — do not compensate for the missing lane here.
