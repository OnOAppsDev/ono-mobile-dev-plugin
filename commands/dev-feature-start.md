...
2a. **Consistency preflight — has the DD moved since this breakdown was generated?** If a Task Breakdown already exists for this feature, compute the DD's body fingerprint and compare it to the breakdown's recorded `source_fingerprint`:

   ```
   node --no-warnings "${CLAUDE_PLUGIN_ROOT}/scripts/task-state.ts" fingerprint --file "<absolute DD path>"
   ```

   Also verify the DD's own `source_fingerprint` against the approved feature analysis it cites. If **that** mismatches, the DD itself is built on a superseded analysis: **stop** and say **"The feature analysis changed after this DD was generated. Re-approve the amended analysis, then run `/dev-design-start`."** Absent on either document means unknown — report it in one line and continue; it is never a mismatch.

2b. **Show the deterministic impact report before rewriting any row.** Read the feature's lifecycle state and classify every recorded task by comparing the current rows against the stored `rowFingerprint` values:
   ...
   Report four buckets — **Unchanged**, **Modified**, **Removed**, **Unrecorded** — and show them **before** step 5 writes anything, so the developer regenerates with the consequences visible rather than blind. This comparison is deterministic: it is set arithmetic over hashes already on disk. Do **not** attempt to diff a freshly regenerated breakdown against the old one — decomposing a DD into rows is model work and two runs are not byte-comparable, so such a diff would report noise as signal.

2c. **If a Task Breakdown or Dev Plan already exists for this feature**, ask how to handle it — `Overwrite` / `Update` (merge new rows, keep recorded ones intact) / `Preserve` (write to a new filename) / `Version` (rename the existing file, e.g. append its date) — the same four options `/dev-design-start` offers for a DD. Never silently overwrite an approved breakdown that has recorded task work against it.
