...
- `verificationDebt` → every entry is verification the plugin could not perform and QA now owns. Carry each into the handoff with its `domain`, `ruleId`, `requiredVerification`, `whyNotAutomatable` and `owner`. Entries are `pending` by construction; the plugin never records them discharged.

These come from the persisted store, so they survive a lost session. Do not ask the feature-developer agent to re-derive accessibility evidence that the store already carries, and do not infer a VoiceOver or TalkBack pass from a recorded mechanical check — `VERIFY-2` forbids it.
