...
Routing diverges most of the base documents, because two premises weaken: **the URL may not be shareable, and the D-pad replaces the pointer.** The rules stand; their rationale shifts.

- **URL-as-source-of-truth holds; the *sharing* rationale often does not.** `REACT-ROUTE-URL-1..4` apply unchanged. What weakens is `REACT-ROUTE-URL-5`'s and `REACT-ROUTE-STABILITY-2`'s premise of externally distributed links: a packaged app is installed locally and its internal URLs are generally not shareable. Judge URL-worthiness on reload-survival and reproducibility, and **do not raise a finding resting on a shareability requirement the surface does not have.**
- **Normally N/A on a packaged app**, recorded with a reason: `REACT-ROUTE-STABILITY-3` (canonical/SEO — nothing crawls it) and the whole `REACT-ROUTE-SSR-*` section (no server render). Both apply unchanged on a browser-based TV surface served over the web.
- **Back is a hardware key and the one control the user always has.** `REACT-TV-INPUT-4` owns its behavior including root-level exit; `REACT-ROUTE-UX-3` (never trap or corrupt history) is the mechanism it relies on, and `REACT-ROUTE-SERVICE-3` (router over raw `history`) keeps it coherent. Where the platform drives Back through the History API by default, a repository that opts out owns the behavior itself.
- **Scroll restoration gives way to focus restoration.** `REACT-ROUTE-UX-2` still applies where the surface scrolls, but the user-visible obligation is `REACT-TV-FOCUS-4`. Restoring scroll while dropping focus is a `REACT-TV-FOCUS-1` defect, not a routing pass.
- **`REACT-ROUTE-UX-1` link semantics apply with the pointer half removed** — a link must be focusable and activatable, "open in a new tab" has no TV meaning, and no affordance may depend on hover, tap, or drag (`REACT-TV-INPUT-6`). `REACT-TV-FOCUS-3` owns the visible focus treatment.
- **`REACT-ROUTE-UX-4` prefetching is weighed against a memory ceiling, not just bandwidth** — prefetch consumes budget governed by `REACT-TV-PERF-1`/`-2`, and any magnitude claim follows the measurement discipline.
- **A launch or relaunch payload is an untrusted navigation input.** `REACT-ROUTE-SECURITY-1` and shared `SEC-WEB-5` already require validating a target taken from user-controllable input; on TV that extends to a vendor launch parameter, owned by `REACT-TV-LIFECYCLE-3`. `REACT-ROUTE-SECURITY-2` applies unchanged.
