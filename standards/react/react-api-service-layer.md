...
Every rule here applies on a TV surface, but **two premises of `REACT-API-BASEQ-2` and `-5` are browser premises** and are established rather than assumed:

- **`REACT-API-BASEQ-2` reads as unconditional; on TV it is conditional on cookie transport working.** A TV platform distinguishes a **packaged** app (resources installed locally, non-`http(s)` scheme) from a **hosted** app served over `http(s)`, and cookie behavior may differ between them. `REACT-TV-API-1` requires establishing which the repository ships and whether cookies are usable, from vendor documentation for the targeted firmware — a Design-stage finding recorded with its source, never an assumption.
- **Where cookies are unavailable, this rule's in-memory fallback becomes the primary design** (`REACT-TV-API-2`), with nothing relaxed: no token in a script-readable store (`REACT-STATE-PERSIST-2`, shared `SEC-COOKIE-2`, `SEC-STORAGE-3`), none logged (shared `SEC-LOG-1`), none inlined into the package (`REACT-API-BASEQ-6`, `REACT-TV-PKG-6`). **Reaching for `localStorage` because "cookies don't work on TV" is a defect, not a workaround** — including mirroring a cookie into it to survive restarts.
- **`REACT-API-BASEQ-5`'s CORS model may not describe the target.** A packaged app's origin is not the API's, so requests are cross-origin by construction; a platform may require reachable origins declared in the **application manifest** and may not send the `Origin` header at all, which breaks any origin-reflecting server. `REACT-TV-API-3` owns establishing the model, so adding a backend host may be a packaging change (`REACT-TV-PKG-3`). Unchanged: CORS is a read boundary, never authorization (shared `SEC-WEB-6`).
- **`REACT-API-BASEQ-3`'s CSRF exposure is re-derived, not inherited.** Protection is required when state changes ride an **ambient cookie**. Where the established transport carries none, classic browser CSRF is not the live threat — a **reasoned N/A against the established transport**, never a passed check, re-derived if the transport changes (`REACT-TV-API-4`).
