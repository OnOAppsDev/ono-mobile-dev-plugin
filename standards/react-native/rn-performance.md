...
- `RN-PERF-RERENDER-1` [WARNING] **This rule is the single authority on _when_ memoization (`React.memo`, `useMemo`, `useCallback`) is required, including for callback props.** Memoization is justified when a non-primitive prop (an object, array, or function) is passed to a child, when a derived calculation is genuinely expensive, or when a heavy render tree would otherwise re-render needlessly. It is **not** required by default — memoization applied reflexively costs more than it saves and is not a finding under any rule here. **Exemptions:** standard primitive UI components (`<Text>`, `<View>`, `<TouchableOpacity>`, and equivalents) receiving scalar primitives (`string`, `number`, `boolean`) or a simple inline handler (a one-line arrow function with no closure over non-primitive state) do NOT require `useCallback`/`useMemo`, unless that component is wrapped in `React.memo` or is rendered inside a list with more than 20 items — in either of those cases the exemption no longer applies and memoization is required. **No other rule in this document may require memoization that this rule exempts**; where they appear to disagree, this rule governs.
...
- `RN-PERF-RERENDER-4` [WARNING] Where memoization **is** used — because `RN-PERF-RERENDER-1` requires it, or because the project already chose it — its dependency array is complete and accurate: every value the callback or computation closes over appears in it, and none is omitted to suppress a re-run. A `useCallback`/`useMemo` carrying a wrong or truncated dependency array is worse than none, because it silently serves a stale closure. This rule governs the correctness of memoization that exists; **whether memoization is required at all is `RN-PERF-RERENDER-1`'s decision alone**, and this rule never creates a memoization requirement of its own.
