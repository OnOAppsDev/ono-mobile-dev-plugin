...
**REACT-001 EPIC COMPLETE (2026-08-16). REACT-002 EPIC COMPLETE (2026-08-17). REACT-003 IN PROGRESS — 003-1 and 003-2 complete (2026-08-19); 003-3 remains.**
...
1. **REACT-003-1 adversarial review pass — still owed.** The mechanical checks ran and pass (all 54 cited non-TV IDs resolve to a definition; all 11 referenced repo paths exist; no `RN-*`/`AND-*`/`IOS-*` ID leaked; 13/13 external links fetched and resolving). What has **not** run is the batched adversarial review the acceptance bar's item 9 requires — contract compliance, React/TV technical correctness, neutrality/density, executability — plus the whole-repo false-claim grep. Per the bar this is run **batched per epic**, so it may legitimately wait until 003-2 and 003-3 land and cover all three at once; that is the intended sequencing, not a skipped step.
2. **REACT-003-3** — state the `device_type` contract in the 3 skills + 4 agents. **The exact lines to update are already located** — 15 of them, all currently claiming `REACT-TV-*` is unauthored/not citable:
