...
Four passes run across 003-1/-2/-3 together, per the acceptance bar's per-epic batching. The mechanical checks were already green; the findings below are what the adversarial passes produced. **All nine were fixed on 2026-08-19** (Boris: "fix them all"), including the three that needed a decision — the resolution taken is recorded under each. Re-verified after the fixes: 47 TV rules still (no rule added or removed to fix a finding), every cited ID across the standard, the four base branches, and the seven agents/skills resolves; all skill anchors resolve; the frozen 111 base IDs remain byte-identical to `463fc14`; 13 external links unchanged and no unverified URL introduced.
...
2. **Optional, and the highest-value remaining quality spend:** an **independent** adversarial pass on `standards/react/react-smart-tv.md`. The passes above were run in-context by the author, which is structurally weak on exactly the rules the author is invested in — F4 was a genuinely wrong rule that survived initial authoring and two verification sweeps. Aim a fresh pass at: the base-document branches' **N/A calls** (are `REACT-ROUTE-SSR-*`, `REACT-ARCH-BOUNDARY-*`, `REACT-PERF-HYDRATION-1`, `REACT-NAME-6` genuinely N/A on a packaged TV app, or only usually?); whether the four branches truly author no rules; whether the new collision table is complete rather than merely plausible; and the `REACT-API-BASEQ-2` cookie question in *Deferred* below, which is still open and still reads as unconditional.
