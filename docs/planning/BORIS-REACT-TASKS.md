...
**REACT-001 ✓ (2026-08-16). REACT-002 ✓ (2026-08-17). REACT-003 authored ✓ (2026-08-19) — all three epics authored end to end. ⚠️ **REACT-003 is NOT signed off:** the batched adversarial review ran 2026-08-19 and returned **9 open findings** (see the review section below), three of which need a decision before they can be fixed.
...
## REACT-003 batched adversarial review — RUN 2026-08-19 · 9 findings, all OPEN

Four passes run across 003-1/-2/-3 together, per the acceptance bar's per-epic batching. The mechanical checks were already green; these are the findings the adversarial passes produced. **None is fixed yet** — F1, F2, and F4 need a decision, not just an edit.
...
**Next — resume here**
1. ~~**The REACT-003 batched adversarial review**~~ — ✓ **run 2026-08-19; 9 findings above, all open.** Fix order: F1, F2, F4 first (they need decisions), then F3/F5/F6/F8, then F7/F9. All three subtasks are authored and every *mechanical* check passes (see each subtask above: cited IDs resolve, paths exist, anchors resolve, no foreign IDs, 13/13 external links live, frozen base IDs byte-identical to `463fc14`, and the whole-repo false-claim grep is clean). What has **not** run is the acceptance bar's item 9 — the four adversarial passes: **contract compliance** (does every deliverable honor the lane conventions, the applicability/N-A discipline, and the routing table?), **React/TV technical correctness** (are the TV claims true, and is the vendor-neutrality real rather than nominal?), **neutrality & density** (any smuggled stack default, any modernization aside, any padding?), and **executability** (could an agent actually follow this on a real repo without stalling?). Now is the right moment: it is scoped to cover 003-1, 003-2, and 003-3 together, which is exactly the per-epic batching the bar prescribes.
   - Highest-value things to aim it at, based on what authoring surfaced: the four base-document branches' **N/A calls** (are `REACT-ROUTE-SSR-*`, `REACT-ARCH-BOUNDARY-*`, `REACT-PERF-HYDRATION-1`, `REACT-NAME-6` genuinely N/A on a packaged TV app, or only usually?); whether the branches truly author no rules; whether a reviewer can actually apply the TV **applicability gate** from a diff alone; and the `REACT-API-BASEQ-2` cookie question in *Deferred* below.
...
- ⚠️ **003-2's document list may be two documents short — Boris's call, not taken here.** The subtask table names exactly four base documents (`react-performance / -coding-standards / -architecture / -routing`), and all four are done. But `react-state-management.md` and `react-api-service-layer.md` have **genuine TV divergences that now have no branch**, and the TV standard already cross-references into both, so the pointer is one-directional:
  - `react-api-service-layer.md` — **the strongest case.** `REACT-API-BASEQ-2`'s auth-transport rule assumes a browser origin where `httpOnly`/`Secure`/`SameSite` cookies behave normally. On a packaged Tizen/webOS app loaded from a local or vendor-specific origin, **those cookie assumptions may not hold at all**, which would change the recommended token transport on TV. Nothing in the lane currently says so. Also unbranched: `REACT-API-ERR-*` as the path for player errors (`REACT-TV-MEDIA-5`) and `REACT-API-BASEQ-6` secrets ↔ `REACT-TV-PKG-6`.
  - `react-state-management.md` — `REACT-STATE-BOUNDARY-2`'s URL-appropriateness weakens on TV for the same shareability reason handled in the routing branch; `REACT-STATE-PERSIST-1/2` need a TV storage reading; and `REACT-TV-FOCUS-4` places focus-restoration state via `REACT-STATE-BOUNDARY-*` without the reverse pointer.
  - **Not widened unilaterally** — 003-2's scope is a defined subtask in a shared plan, and the auth-transport question in particular needs a real answer about cookie behavior on a packaged origin (verify against vendor docs; do not assume). Options: extend 003-2 to six documents, or log it as a REACT-004. The cookie-behavior question should be settled either way, since `REACT-API-BASEQ-2` currently reads as unconditional.
