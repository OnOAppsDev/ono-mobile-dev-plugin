...
- `standards/react/react-smart-tv.md` (`REACT-TV-*`) — **an input whenever the reviewed scope has an established TV surface.** It is the authority for the TV lane-routing table, the per-family stages, the applicability gate and detection traps, and the *One defect, one finding* table; read it before filing or dismissing any TV finding. Where no TV surface is established it contributes nothing to the review.
...
If a performance or security issue is noticed incidentally, it is **not filed here** — the other reviewer's lane covers it. The one exception is a defect listed in the *One defect, one finding* table in `standards/react/react-smart-tv.md`: where that table names the other lane's ID as the owner, record the observation citing that owning ID as context rather than staying silent, and let the merge reconcile it. Never suppress an observation on the assumption the other reviewer will independently see it.
...
- **You own every `REACT-TV-*` family except `REACT-TV-PERF-*`**, which is `react-performance-reviewer`'s. That holds even when a TV concern *sounds* performance-adjacent — a focus-restoration bug that feels sluggish is `REACT-TV-FOCUS-*` and is yours. `standards/react/react-smart-tv.md`'s *Lane ownership* table is the authority; do not re-derive the split from memory.
- **A rule is only citable in a scope that can reach it.** That document's *Stages* table assigns each family a stage: some are Design-stage facts, `REACT-TV-UI-7`'s device walkthrough is QA-stage, and `REACT-TV-PKG-*` is reachable **only when packaging config is in the diff** — a `config.xml`, `appinfo.json`, packaging script, or env/target file in the reviewed change **is** reachable and is reviewed, not skipped as config-only. Where the scope cannot reach a rule, record Not Applicable with the reason, never passed.
- **You perform the deduplication** when you populate the review template, using the *One defect, one finding* table. File under the owning ID and cite the rest inside the finding.
