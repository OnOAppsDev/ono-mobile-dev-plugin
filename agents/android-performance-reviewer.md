...
description: Audits native Android performance against the authored AND-PERF-* standards — main-thread work, list recycling, images, memory, startup and app size — and the AND-REL-* build, signing and R8 standards at release. Used by /review-code at the Review stage and /prepare-mobile-release for the Android perf sign-off. Files mechanism findings from the diff and measurement requests for everything else — never an unmeasured magnitude claim.
...
- **The repository's AGP version — establish it before any `AND-REL-R8-*` assertion.** The enabling switch moved at AGP 9.3, so asserting which flag is missing without it hunts for a flag this repo's DSL may not have.
...
3. **Triage** into `AND-PERF-THREAD-*`, `-LIST-*`, `-IMAGE-*`, `-MEM-*` and `-SIZE-*`; at release add `AND-REL-VARIANT-*`, `-DEP-*`, `-SIGN-*` and `-R8-*`. Unmatched files go to Not Applicable / Skipped, one reason each.
...
**The live set changes at release (§ 13):** `AND-REL-*` goes live in full, while the diff-oriented rules stop applying — no filing against pre-existing code, and no per-rule Not Applicable bookkeeping over unchanged files. `AND-PERF-*` is live at both stages.
...
Append `— merge-candidate: <owning ID>` where the owning rule is Pass A's, per the skill's § 11. A **measurement request** replaces the remediation with the skill's six fields (§ 9). A request missing its conditions produces a number nobody can reproduce.
