...
- **Android**: lightweight existence checks only (Gradle Kotlin DSL vs. Groovy, Compose vs. XML view presence) — **deliberately so**: `android-architect` runs its own deeper Android inspection per `skills/android-dev-planning/SKILL.md`'s repository-evidence-collection step rather than relying on this step.
