...
- On `device_type: tv`, the **`AND-*TV-*` rules those same ten files now carry**, each inside its host document's `## Android TV Context (device_type: tv)` section: `AND-UI-TV-*` (`compose-xml-standards.md`), `AND-NAV-TV-*` (`android-navigation.md`), `AND-ARCH-TV-*` (`android-architecture.md`), `AND-PERF-TV-*` (`android-performance.md`), `AND-REL-TV-*` (`gradle-build-signing.md`). **`AND-TEST-INSTR-2` is the exception and is addressed differently:** it carries no `TV` segment and sits in `android-testing.md`'s existing `## Instrumentation & UI Tests` section, **not** in a TV context section — that file has none, deliberately, because no `AND-TEST-TV-*` family was opened. There is no separate TV standards file and **no TV root outside this set** — skill §8's citation table is the authority.
...
3. **Resolve canonical repository knowledge before deriving anything**, through `repo-knowledge-consumer`, which owns the resolution procedure — it arrives already resolved for this run, so never re-derive a reusable category, and never take `device_type` from the repository-knowledge manifest, which carries none (skill §2).
4. **Inspect before proposing** — work through the dimensions in skill §3 in its stated order and only to the depth the feature actually touches, **and on `device_type: tv` run the skill's §6 TV discovery pass in the same sweep**, apply its detection traps, label every finding `[evidence: …]` / `[reused: …#anchor]` / `[inference]` / `[unknown]`, and record the change surface during that same sweep rather than in a second pass. **Detect — never assume.**
...
- **Never treat TV as a separate platform** — `device_type: tv` is a context signal handled by this same agent, and there is no TV platform value, TV agent or TV command. **On `device_type: tv`, cite the `AND-*TV-*` rules in addition to the base `AND-*` rules, which all still apply on a TV surface unchanged** — never instead of them, and never on a `mobile` plan. Confirm an ID in the host document that owns it before citing it, and record a genuinely unstandardised TV question as an unresolved decision rather than stretching an adjacent rule (skill §6, skill §8).
- **Never silently apply mobile/touch assumptions to a `tv` plan** — most TV devices have no touchscreen or pointer and rely on a D-pad remote, so a D-pad path is always required and touch is never assumed (`AND-NAV-TV-DPAD-1`). Focusability and a visible focus state stay credited to `A11Y-TOUCH-1` and `A11Y-TOUCH-2`, which already carry Android-TV clauses; cite those two rather than restating them as a TV rule.
...
One more that is specific to `tv`: a TV surface **exists** and its implementation model cannot be identified from evidence. **A repository with no TV surface at all is not this stop** — that is the greenfield path in the skill's §6, which is planned rather than refused. A manifest declaration identifies a TV **target**; **nothing identifies the TV UI framework except reading the repository**, and this organisation's TV surface uses a custom in-house framework — so the absence of a well-known TV library settles nothing, is never evidence against a TV surface, and is never grounds to propose adopting or migrating to one.
